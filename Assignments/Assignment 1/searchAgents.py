...
class CapsuleSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        self.capsules = tuple(startingGameState.getCapsules())
        self.startState = (startingGameState.getPacmanPosition(), self.food, self.capsules)
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE FOR TASK 3 ***"
        return len(state[2]) == 0 and state[1].count() == 0
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[newx][newy]:
                if len(capsule_left) != 0:
                    if not new_food[newx][newy]:
                        capsules = list(state[2])
                        for cur_cap in capsules:
                            if cur_cap[0] == newx and cur_cap[1] == newy:
                                capsules.remove(cur_cap)
                        successors.append((((newx, newy), new_food, tuple(capsules)), direction, 1))
                else:
                    new_food[newx][newy] = False
                    successors.append((((newx, newy), new_food, ()), direction, 1))
        return successors
...
def manHantDis(pos1, pos2):
    return abs(pos2[0] - pos1[0]) + abs(pos2[1] - pos1[1])

def greedyHeuristic(coordinate, foodlist):
    if len(foodlist) == 0:
        return 0
    ...
    for food in foodlist:
        dis = manHantDis(coordinate, food)
        if dis < min_dis:
            min_dis = dis
            min_food = food
    next_foodlist.remove(min_food)
    return min_dis + 1.2*greedyHeuristic(min_food, next_foodlist)
...
def capsuleProblemHeuristic(state, problem):
    ...
    """
    "*** YOUR CODE HERE FOR TASK 3 ***"
    coordinate, food, capsules = state
    foodlist = food.asList()

    if len(capsules) != 0:
        return manHantDis(coordinate, capsules[0]) + greedyHeuristic(capsules[0], foodlist)
    else:
        return greedyHeuristic(coordinate, foodlist)
